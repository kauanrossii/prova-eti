...
public class AdicionarDestinoViagemCommandHandler : IUseCase<AdicionarDestinoViagemCommand, EntityIdDto>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var destinoCadastrado = await _context.Destinos.FindAsync(requisicao.DestinoId);
        ...
        return !sucessoOperacao
            ? ResultDto<EntityIdDto>.ComErro(["O destino selecionado já foi adicionado à viagem."])
            : ResultDto<EntityIdDto>.ComSucesso(new EntityIdDto(destinoCadastrado.Id));
    }
}